...
function setup() {
	...
	add_filter( 'block_editor_settings', $n( 'register_block_templates' ), 10, 2 );
...
function register_theme_blocks() {
	...
	if ( file_exists( WRT_THEME_BLOCK_DIST_DIR ) ) {
		$block_json_files = glob( WRT_THEME_BLOCK_DIST_DIR . '*/block.json' );
...
function blocks_editor_styles() {
	wp_enqueue_style(
		...
		WRT_THEME_TEMPLATE_URL . '/dist/css/editor-style-overrides.css',
		[],
	...
	if ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) {
		wp_enqueue_script(
			'editor-style-overrides',
			WRT_THEME_TEMPLATE_URL . '/dist/js/editor-style-overrides.js',
...
function register_block_pattern_categories() {
	...
	register_block_pattern_category(
		'10up-theme',
		[ 'label' => __( 'WeAreTeachers - General', 'wrt-theme' ) ]
	);

	// Register a block pattern category
	register_block_pattern_category(
		'10up-theme-microsite',
		[ 'label' => __( 'WeAreTeachers - Microsite', 'wrt-theme' ) ]
	);
}
 ...
 *
 * This registers the block template for the section hub pages.
 *
 * @param array $settings Settings array.
 * @param mixed $post     Post.
 */
function register_block_templates( $settings, $post ) {
	if (
		'page' === get_post_type( $post )
		&& 'page-section-hub' === basename( get_page_template_slug( $post ), '.php' )
	) {
		$settings['templateLock'] = 'all';
		$settings['template']     = array(
			array(
				'tenup/curated-content',
				array(
					'layout' => array(
						'type' => 'list',
					),
