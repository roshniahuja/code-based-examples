...
class ModuleInitialization {
	...
	/**
	 * Get all the WRTPlugin plugin classes.
	 *
	 * @return array
	 */
	protected function get_classes() {
		$class_finder = new ClassFinder();
		$class_finder::setAppRoot( WRT_PLUGIN_PATH );
		return $class_finder::getClassesInNamespace( 'WRTPlugin', ClassFinder::RECURSIVE_MODE );
	}

	/**
	 * Initialize all the WRTPlugin plugin classes.
	 *
	 * @return void
	 */
	public function init_classes() {
		...
		foreach ( $this->get_classes() as $class ) {
			...
			if ( ! $reflection_class->isSubclassOf( '\WRTPlugin\Module' ) ) {
				continue;
			}
